...
def call_Drehen_right_on(event):         #When this function is called,client commands the car to move forward
    global c_x_stu
    if c_x_stu == 0:
        tcpClicSock.send(('drehen_right').encode())
        c_x_stu=1

def call_Drehen_left_on(event):         #When this function is called,client commands the car to move forward
    global c_x_stu
    if c_x_stu == 0:
        tcpClicSock.send(('drehen_left').encode())
        c_x_stu=1
      
def call_Vorwaerts(event):         #When this function is called,client commands the car to move forward
    global c_f_stu
    if c_f_stu == 0:
        tcpClicSock.send(('forward').encode())
        c_f_stu=1        

def call_Rueckwaerts(event):         #When this function is called,client commands the car to move forward
    global c_b_stu 
    if c_b_stu == 0:
        tcpClicSock.send(('backward').encode())
        c_b_stu=1   

def call_Drehen_right(event):
    if Drehen_right == 0:
        tcpClicSock.send(('rightside').encode())

def call_Drehen_left(event):
    if Drehen_left == 0:
...
def call_Stop(event):         
    ...
    global c_ls_stu,c_rs_stu,c_x_stu
    c_f_stu = 0
    c_b_stu = 0
    c_l_stu=0
    c_r_stu=0
...
def connection_thread():
    global funcMode, Diagonal, Switch_4, Switch_3, Switch_2, Switch_1, SmoothMode, Drehen_left, Drehen_right
    while 1:
...
def Info_receive():
    ...
    HOST = ''
    INFO_PORT = 2256
#    INFO_PORT = 10223 #       Define port serial 
    ADDR = (HOST, INFO_PORT)
...
def loop():                   #GUI
    ...
    while True:
        ...
        Btn_Smooth.place(x=480,y=295)
        Btn_Smooth.bind('<ButtonPress-1>', call_Smooth)
        root.bind('<KeyPress-f>', call_Smooth)

        Btn_Drehen_left_on = tk.Button(root, width=8, text='LinksDrehen',fg=color_text,bg=color_btn,relief='ridge')
        Btn_Drehen_left_on.place(x=30,y=190)
        Btn_Drehen_left_on.bind('<ButtonPress-1>', call_Drehen_left_on)

        Btn_Stop = tk.Button(root, width=8, text='Stop',fg=color_text,bg=color_btn,relief='ridge')
        Btn_Stop.place(x=120,y=190)
        Btn_Stop.bind('<ButtonPress-1>', call_Stop)
        root.bind('<KeyPress-f>', call_Stop)

        Btn_Drehen_right_on = tk.Button(root, width=8, text='RechtsDrehen',fg=color_text,bg=color_btn,relief='ridge')
        Btn_Drehen_right_on.place(x=210,y=190)
        Btn_Drehen_right_on.bind('<ButtonPress-1>', call_Drehen_right_on)
        
        Btn_Drehen_left = tk.Button(root, width=8, text='Links',fg=color_text,bg=color_btn,relief='ridge')
        Btn_Drehen_left.place(x=30,y=225)
        Btn_Drehen_left.bind('<ButtonPress-1>', call_Drehen_left)
        root.bind('<KeyPress-f>', call_Drehen_left)

        Btn_Drehen_right = tk.Button(root, width=8, text='Rechts',fg=color_text,bg=color_btn,relief='ridge')
        Btn_Drehen_right.place(x=210,y=225)
        Btn_Drehen_right.bind('<ButtonPress-1>', call_Drehen_right)
        root.bind('<KeyPress-f>', call_Drehen_right)
        
        Btn_Gerade = tk.Button(root, width=8, text='Vorwaerts',fg=color_text,bg=color_btn,relief='ridge')
        Btn_Gerade.place(x=120,y=155)
        Btn_Gerade.bind('<ButtonPress-1>', call_Vorwaerts)
        root.bind('<KeyPress-f>', call_Turn_stop)

        Btn_Gerade = tk.Button(root, width=8, text='Rueckwaerts',fg=color_text,bg=color_btn,relief='ridge')
        Btn_Gerade.place(x=120,y=225)
        Btn_Gerade.bind('<ButtonPress-1>', call_Rueckwaerts)
        root.bind('<KeyPress-f>', call_Turn_stop)
        ...
        Btn_RightSide.place(x=210,y=260)
        ...
        #Btn_up.place(x=400,y=225)
        Btn_up.place(x=390,y=225)
        Btn_down.place(x=390,y=295) 
        Btn_left.place(x=300,y=260)
        Btn_right.place(x=480,y=260) 
        Btn_home.place(x=390,y=260)
