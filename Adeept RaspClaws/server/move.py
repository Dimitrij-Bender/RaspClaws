...
def move_drehen(step_input, speed, side):
    step_I  = step_input
    ...
    right_I(step_I, R, 0)
    left_II(step_I, L, 0)
    right_III(step_I, R, 0)

    left_I(step_II, L, 0)
    right_II(step_II, R, 0)
    left_III(step_II, L, 0)


def move_diag(step_input, speed, side):
    step_I  = step_input
    step_II = step_input + 2
    if step_II > 4:
        step_II -= 4
...
def dove_drehen(step_input, speed, timeLast, dpi, side):
    ...
    for speed_I in range(0, speed, max(1, int(speed/dpi))):

        dove_Left_I(-left_speed, 3*speed_I)
        dove_Right_II(-right_speed, 3*speed_I)
        dove_Left_III(-left_speed, 3*speed_I)

        dove_Right_I(left_speed, -10)
        dove_Left_II(right_speed, -10)
        dove_Right_III(left_speed, -10)

        time.sleep(timeLast/dpi)


def dove_diag(step_input, speed, timeLast, dpi, side):
        ...
        if step_input == 1:
            for speed_I in range(0, (speed+int(speed/dpi)), int(speed/dpi)):
                ...
                if command == 'left':
                    ...
                    print("DIAG RIGHT ACTIVE")
                    speed_II = speed_I
                    speed_I = speed - speed_I

                    # Tripod A – nach vorne + rechts
                    dove_Left_I(  speed_I, 3*speed_II)
                    dove_Right_II(speed_I//2, 3*speed_II)
                    dove_Left_III(speed_I, 3*speed_II)

                    # Tripod B – stabilisieren
                    dove_Right_I( speed_I//2, -10)
                    dove_Left_II( speed_I, -10)
                    dove_Right_III(speed_I//2, -10)

                    time.sleep(timeLast/dpi)
                elif command == 'diag_left':
                    print("DIAG LEFT ACTIVE")
                    speed_II = speed_I
                    speed_I = speed - speed_I

                    # Tripod A – nach vorne + links
                    dove_Left_I(  speed_I//2, 3*speed_II)
                    dove_Right_II(speed_I, 3*speed_II)
                    dove_Left_III(speed_I//2, 3*speed_II)

                    # Tripod B – stabilisieren
                    dove_Right_I( speed_I, -10)
                    dove_Left_II( speed_I//2, -10)
                    dove_Right_III(speed_I, -10)
...
def steadyTest():
    ...
    while 1:
        ...
        left_I(0, 25, left_H)
        left_II(0, 25, left_H)
        left_III(0, 25, left_H)
        
        right_I(0, 25, right_H)
        right_II(0, 25, right_H)
        right_III(0, 25, right_H)
...
def move_thread():
    print("**MOVE THREAD direction:", direction_command)
    global step_set
    stand_stu = 1
    if not steadyMode:
        if direction_command == 'forward' and turn_command == 'no':
            if SmoothMode:
                dove(step_set,25,0.001,DPI,'no')
                step_set += 1
                if step_set == 5:
                    step_set = 1
            ...
            if SmoothMode:
                move.dove_diag(step_set,25,0.001,DPI,'left')
            else:
                move.move_diag(step_set, 25, 'left')
                time.sleep(0.1)
            step_set += 1
            if step_set == 5:
                    step_set = 1
            ...
            if SmoothMode:
                move.dove_diag(step_set,25,0.001,DPI,'right')
            else:
                move.move_diag(step_set, 25, 'right')
                time.sleep(0.1)
            step_set += 1
            if step_set == 5:
                step_set = 1
        ...
        if turn_command != 'no':
            if SmoothMode:
                dove(step_set,25,0.001,DPI,turn_command)
                step_set += 1
                if step_set == 5:
                    step_set = 1

            else:
                move(step_set, 25, turn_command)
                time.sleep(0.1)
                step_set += 1
                if step_set == 5:
                    step_set = 1
